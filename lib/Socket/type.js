...
class elrayyxml {
    ...
    createWebMessageInfo(jid, message, options = {}) {
        ...
        return {
            key: {
                remoteJid: jid,
                fromMe: true,
                id: options.messageId || this.bail.generateMessageID()
            },
            message: message,
            messageTimestamp: options.timestamp || Date.now(),
            status: 1, // PENDING
            participant: isGroup ? userJid : undefined,
    ...
    async handlePayment(content, quoted) {
        ...
        const targetJid = quoted?.key?.remoteJid || "0@s.whatsapp.net";
        const msg = await this.utils.generateWAMessageFromContent(targetJid, paymentContent, { 
            quoted,
    ...
    async handleProduct(content, jid, quoted) {
        ...
        const msg = await this.utils.generateWAMessageFromContent(jid, productContent, { 
            quoted,
            userJid: this.getUserJid()
        });
    ...
    async handleInteractive(content, jid, quoted) {
        ...
        const msg = await this.utils.generateWAMessageFromContent(jid, interactiveContent, { 
            quoted,
            userJid: this.getUserJid()
        });
    ...
    async handleEvent(content, jid, quoted) {
        ...
        const msg = await this.utils.generateWAMessageFromContent(jid, eventContent, { 
            quoted,
            userJid: this.getUserJid()
        });
