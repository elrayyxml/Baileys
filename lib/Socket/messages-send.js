...
const makeMessagesSocket = (config) => {
    ...
    const type = new type_1.type(Utils_1, waUploadToServer, relayMessage);
    ...
    return {
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            const messageType = type.detectType(content);
            
            if (messageType) {
                switch(messageType) {
                    case 'PAYMENT':
                        const paymentContent = await type.handlePayment(content, quoted);
                    ...
                    case 'PRODUCT':
                        const productContent = await type.handleProduct(content, jid, quoted);
                    ...
                    case 'INTERACTIVE':
                        const interactiveContent = await type.handleInteractive(content, jid, quoted);
                    ...
                    case 'ALBUM':
                        return await type.handleAlbum(content, jid, quoted);
                    
                    case 'EVENT':
                        return await type.handleEvent(content, jid, quoted);
                    
                    case 'POLL_RESULT':
                        return await type.handlePollResult(content, jid, quoted);
                    
                    case 'GROUP_STORY':
                        return await type.handleGroupStory(content, jid, quoted);
                    }
                }
