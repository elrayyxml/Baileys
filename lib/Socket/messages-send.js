...
const makeMessagesSocket = (config) => {
    ...
    return {
        ...
        return parseInt(`0xFF${hexColor}`, 16);
    };
    ...
    if (waMsgContent.message) {
        const messageType = Object.keys(waMsgContent.message)[0];
        if (waMsgContent.message[messageType]) {
            const messageContent = waMsgContent.message[messageType];
            
            const isMedia = content.image || content.video || content.audio || content.document || content.sticker;
            const isAudio = !!content.audio;
            
            const font = !isMedia ? (content.font !== undefined ? content.font : Math.floor(Math.random() * 9)) : undefined;
            
            const textColor = !isMedia ? (content.textColor ? 
                (typeof content.textColor === 'string' ? hexToArgb(content.textColor) : content.textColor) : 
                hexToArgb(getRandomHexColor())) : undefined;
            
            const backgroundColor = (!isMedia || isAudio) ? (content.backgroundColor ? 
                (typeof content.backgroundColor === 'string' ? hexToArgb(content.backgroundColor) : content.backgroundColor) : 
                hexToArgb(getRandomHexColor())) : undefined;
            
            if (isMedia && !isAudio) {
                delete waMsgContent.message[messageType].backgroundColor;
                delete waMsgContent.message[messageType].textColor;
                delete waMsgContent.message[messageType].font;
            }
            
            if (!isMedia || isAudio) {
                if (messageType === 'extendedTextMessage') {
                    waMsgContent.message[messageType] = {
                        ...messageContent,
                        ...(backgroundColor !== undefined && { backgroundArgb: backgroundColor }),
                        ...(textColor !== undefined && { textArgb: textColor }),
                        ...(font !== undefined && { font })
                    };
                } else if (messageType === 'conversation' && content.text) {
                    waMsgContent.message = {
                        extendedTextMessage: {
                            ...
                            ...(textColor !== undefined && { textArgb: textColor }),
                            ...(font !== undefined && { font }),
                            ...(messageContent.contextInfo && { contextInfo: messageContent.contextInfo })
                        }
                    };
    ...
    for (const id of jids) {
        try {
            ...
            await Utils_1.delay(2000);
                 ...
                 logger.error(`Error sending mention to ${id}: ${error}`);
               }
            }
